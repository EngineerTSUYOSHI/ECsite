    ...
    <main>
        ...
        <form action='/EC_site/ProductListController' method='post' name='myform'>
	        <div>
	            ...
	            <label>カテゴリ：</label>
	            <!-- カテゴリのプルダウンリストを表示 -->
	            <select name="category_code" id="category_code" onchange='setRecommend(<%=dto.getSearch().getCategoryCode()%>)'>
        ...
        <div class="my-parts">
	        ...
	        <form action='/EC_site/PurchaseController' method='post' name='purchase'>
	        	...
	        	<input type='hidden' value='<%=dto.getSearch().getProductName() %>' name='searchName'>
	        	<input type='hidden' value='<%=dto.getSearch().getCategoryCode() %>' name='searchCategoryCode'>
	        	<input type='hidden' value="<%=lowPrice %>" name='searchLowPrice'>
	        	<input type='hidden' value="<%=upPrice %>" name='searchUpPrice'>
	        	<input type='hidden' value='<%=dto.getSearch().getRecommendCode() %>' name='searchRecommendCode'>
	        	<input type='hidden' value='<%=dto.getNow_page() %>' name='searchNowPage'>
	        </form>
        </div>
    </main>
    <footer>
    ...
    <script>
		...
		function checkChar(priceType){
	        ...
	        if(!regex.test(priceInput.value)){
	            ...
	            priceInput.value="";
	            return;
	        }else if(parseInt(priceInput.value.length) > 7){
	        	alert("7桁以内で入力して下さい");
	            priceInput.value="";
	            return;
	        }
